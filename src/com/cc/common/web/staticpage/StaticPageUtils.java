package com.cc.common.web.staticpage;
...
public class StaticPageUtils {
	...
	// ${contentId}
	// ${page}
	public static String staticUrlRule(String rule, java.lang.Integer modelId, String modelPath, java.lang.Integer channelId, String channelPath, java.lang.Integer contentId, Date date) {
		...
		if (channelId != null) {
			rule = StringUtils.replace(rule, "${channelId}", channelId.toString());
		}
		if (StringUtils.isBlank(channelPath)) {
			rule = StringUtils.replace(rule, "${channelPath}", channelPath);
		}
		if (contentId != null) {
			rule = StringUtils.replace(rule, "${contentId}", contentId.toString());
		}
		if (date != null) {
			// 获得年月日
