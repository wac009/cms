...
public class JsonWriter {
	...
	public String write(Object jsonRoot, Object recordRoot, String records, String fields, String id, String total, Collection<Pattern> excludeProperties)
		...
		if (fields != null && records != null) {
			...
			if (recordRoot instanceof Pagination) {
				...
				serializeList(records, fields, id, total, page.getTotalCount(), page.getList());
			...
			buf.append('}');
		} else if (fields != null && records == null && recordRoot instanceof List) {
			buf.setLength(0);
			List list = (List) recordRoot;
			serializeList(records, fields, id, total, list.size(), list);
		} else {}
	...
	private void serializeList(String records, String fields, String id, String total, long totalCount, List list) {
		if (total != null) {
			buf.append('"').append(total).append("\":").append(totalCount).append(',');
		}
		if (records != null) {
			buf.append('"').append(records).append("\":[");
		} else {
	...
	private void process(Object root, String name, String field) {
		try {
			...
			log.warn("field '" + field + "' not exist in " + root.getClass().getName());
		}
	}
