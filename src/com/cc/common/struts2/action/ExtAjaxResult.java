...
public class ExtAjaxResult implements Result {
	...
	public List<Pattern> excludeProperties(String commaDelim) {
		...
		List<String> excludePatterns = (List<String>) JSONUtil.asSet(commaDelim);
	...
	public void execute(ActionInvocation invocation) throws Exception {
		...
		try {
			...
			if (log.isDebugEnabled()) {
				...
				b.append("records=").append(records).append("\tid=").append(id).append("\ttotal=").append(total).append("\nfields=")
						.append(fields);
				log.debug(b.toString());
			}
			JsonWriter writer = new JsonWriter();
			String json = writer.write(jsonObject, recordObject, records, fields, id, total, null);
