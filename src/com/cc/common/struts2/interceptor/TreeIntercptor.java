...
public class TreeIntercptor extends EmptyInterceptor implements ApplicationContextAware {
	...
	protected SessionFactory getSessionFactory() {
		if (sessionFactory == null) {
			sessionFactory = appCtx.getBean(SESSION_FACTORY, SessionFactory.class);
			if (sessionFactory == null) {
	...
	public boolean onSave(Object entity, Serializable id, Object[] state, String[] propertyNames, Type[] types) {
		if (entity instanceof IHibernateTree) {
			IHibernateTree<?> tree = (IHibernateTree<?>) entity;
	...
	public boolean onFlushDirty(Object entity, Serializable id, Object[] currentState, Object[] previousState, String[] propertyNames,
		...
		for (int i = 0; i < propertyNames.length; i++) {
			if (propertyNames[i].equals(tree.getParentName())) {
				IHibernateTree<?> preParent = (IHibernateTree<?>) previousState[i];
				IHibernateTree<?> currParent = (IHibernateTree<?>) currentState[i];
				return updateParent(tree, preParent, currParent);
			}
		}
		return false;
	}
	private boolean updateParent(IHibernateTree<?> tree, IHibernateTree<?> preParent, IHibernateTree<?> currParent) {
		// 都为空、或都不为空且相等时，不作处理
