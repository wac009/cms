...
import com.cc.common.page.IPaginable;
import com.cc.common.page.SimplePage;
...
@Repository
public class StaticPageDaoImpl extends HibernateSimpleDao implements IStaticPageDao {
//	@Autowired
//	private IKeywordSvc cmsKeywordMng;
	...
	public int channelStatic(Integer siteId, Integer channelId, boolean containChild, Configuration conf, Map<String, Object> data) throws IOException, TemplateException {
		...
		while (channels.next()) {
			...
			if (!c.getModel().getHasContent() || (!c.getListChild() && childs > 0)) {
				totalPage = 1;
			} else {
				if (c.getListChild()) {
					quantity = childs;
	...
	public int contentsOfChannel(Integer channelId) {
		String hql = "select count(*) from Content bean" + " join bean.channels as channel" + " where channel.id=:channelId and bean.status=" + ContentCheck.CHECKED;
	...
	public int contentStatic(Integer siteId, Integer channelId, Date start, Configuration conf, Map<String, Object> data) throws IOException, TemplateException {
		...
		while (contents.next()) {
			...
			for (int pageNo = 1; pageNo <= totalPage; pageNo++) {
				...
				IPaginable pagination = new SimplePage(pageNo, 1, c.getPageCount());
