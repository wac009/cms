...
public abstract class AbstractContentDirective implements TemplateDirectiveModel {
	...
	@Autowired
	protected IContentTagSvc	contentTagMng;
	@Autowired
	protected IChannelSvc		channelMng;
	@Autowired
	protected IWebsiteSvc		cmsSiteMng;
	@Autowired
	protected IContentSvc		contentMng;
	...
	protected Object getData(Map<String, TemplateModel> params, Environment env) throws TemplateException {
		...
		if (channelPaths != null) {
			...
			if (siteIds == null || siteIds.length == 0) {
				List<Site> siteList = cmsSiteMng.getAllWebsites();
				if (siteList.size() == 1) {
					pathsToIds = true;
