...
public class MessageAct {
	...
	@Autowired
	private IMessageSvc			messageMng;
	@Autowired
	private IReceiverMessageSvc	receiverMessageMng;
	@Autowired
	...
	public String message_mng(Integer box, String msg, HttpServletRequest request, HttpServletResponse response, ModelMap model) {
		...
		return FrontUtils.getTplPath(request, site.getTplPath(), TPLDIR_MESSAGE, MESSAGE_MNG);
	}
	...
	public String message_inbox(Integer pageNo, String title, Date sendBeginTime, Date sendEndTime, Boolean status, Integer box, HttpServletRequest request,
		...
		model.addAttribute("box", box);
		return FrontUtils.getTplPath(request, site.getTplPath(), TPLDIR_MESSAGE, returnPage);
	}
	...
	public String message_add(HttpServletRequest request, HttpServletResponse response, ModelMap model) {
		...
		return FrontUtils.getTplPath(request, site.getTplPath(), TPLDIR_MESSAGE, MESSAGE_ADD);
	}
	...
	public String message_reply(Integer id, HttpServletRequest request, HttpServletResponse response, ModelMap model) {
		...
		return FrontUtils.getTplPath(request, site.getTplPath(), TPLDIR_MESSAGE, MESSAGE_REPLY);
	}

	// 直接发送
	@RequestMapping(value = "/member/message_send.jspx")
	public String message_send(Message message, String username, String captcha, String nextUrl, HttpServletRequest request, HttpServletResponse response,
			ModelMap model) {
		Site site = CmsUtils.getSite(request);
		User user = CmsUtils.getUser(request);
		FrontUtils.frontData(request, model, site);
		...
		User msgReceiverUser = userMng.findByUsername(username);
		...
		log.info("member Message save Message success. id={}", message.getId());
		return FrontUtils.showSuccess(request, model, nextUrl);
	}

	// 存草稿
	@RequestMapping(value = "/member/message_save.jspx")
	public String message_save(Message message, String username, String captcha, String nextUrl, HttpServletRequest request, HttpServletResponse response,
			ModelMap model) {
		Site site = CmsUtils.getSite(request);
		User user = CmsUtils.getUser(request);
		FrontUtils.frontData(request, model, site);
	...
	public String message_edit(Integer id, HttpServletRequest request, HttpServletResponse response, ModelMap model) {
		...
		model.addAttribute("message", message);
		return FrontUtils.getTplPath(request, site.getTplPath(), TPLDIR_MESSAGE, MESSAGE_EDIT);
	}

	@RequestMapping(value = "/member/message_update.jspx")
	public String message_update(Message message, String nextUrl, String captcha, HttpServletRequest request, HttpServletResponse response, ModelMap model) {
		Site site = CmsUtils.getSite(request);
		User user = CmsUtils.getUser(request);
		FrontUtils.frontData(request, model, site);
		MemberConfig mcfg = site.getConfig().getMemberConfig();
		...
		log.info("member Message update Message success. id={}", message.getId());
		return FrontUtils.showSuccess(request, model, nextUrl);
	}
	...
	public String message_read(Integer id, Integer box, HttpServletRequest request, HttpServletResponse response, ModelMap model) {
		...
		model.addAttribute("box", box);
		return FrontUtils.getTplPath(request, site.getTplPath(), TPLDIR_MESSAGE, MESSAGE_READ);
	}
	...
	public void findUnreadMessagesByUser(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws JSONException {
		...
		if (user == null) {
			...
			List<ReceiverMessage> receiverMessages = receiverMessageMng.getList(site.getId(), null, user.getId(), null, null, null, false, 0, false);
			object.put("result", true);
			if (receiverMessages != null && receiverMessages.size() > 0) {
