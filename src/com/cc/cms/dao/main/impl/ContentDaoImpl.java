...
import static com.cc.cms.entity.main.Content.ContentStatus.all;
import static com.cc.cms.entity.main.Content.ContentStatus.checked;
import static com.cc.cms.entity.main.Content.ContentStatus.draft;
import static com.cc.cms.entity.main.Content.ContentStatus.passed;
import static com.cc.cms.entity.main.Content.ContentStatus.prepared;
import static com.cc.cms.entity.main.Content.ContentStatus.recycle;
import static com.cc.cms.entity.main.Content.ContentStatus.rejected;
...
@Repository
public class ContentDaoImpl extends CmsDaoImpl<Content> implements IContentDao {
	@Override
	public Pagination getRightContent(int pageNo, int pageSize, Integer chnlId, List<String> property, List<String> ops, List<Object> value, int order) {
		...
		if (chnlId != null) {
			f.append(" join bean.channels chnls,Channel p");
			f.append(" where chnls.lft between p.lft and p.rgt and p.id=:chnlId ");
			f.setParam("chnlId", chnlId);
		} else {
			f.append(" where 1=1 ");
		}
		appendPropertys(f, property, ops, value, false);
		appendOrder(f, order);
	...
	public Pagination getRightContent(int pageNo, int pageSize, Integer chnlId,Boolean selfChannel, List<String> property, List<String> ops, List<Object> value, int order) {
		Finder f = Finder.create("select bean from Content bean");
		if (chnlId != null) {
			if (selfChannel!=null&&selfChannel) {
				f.append(" join bean.channels chnls,Channel p");
				f.append(" where chnls.lft = p.lft  and p.id=:chnlId ");
				f.setParam("chnlId", chnlId);
			}else {
				f.append(" join bean.channels chnls,Channel p");
				f.append(" where chnls.lft between p.lft and p.rgt and p.id=:chnlId ");
	...
	public Pagination getPage(String title, Integer typeId, Integer inputUserId, boolean topLevel, boolean recommend, ContentStatus status,
		...
		if (prepared == status || passed == status || rejected == status) {
			f.append(" join bean.contentCheck check");
		}
		if (channelId != null) {
	...
	private void appendQuery(Finder f, String title, Integer typeId, Integer inputUserId, ContentStatus status, boolean topLevel, boolean recommend) {
	...
	public Pagination getPageByChannelIdsForTag(Integer[] channelIds, Integer[] typeIds, Boolean titleImg, Boolean recommend, String title,
		...
		Finder f = byChannelIds(channelIds, typeIds, titleImg, recommend, title, orderBy, option);
		f.setCacheable(true);
		return find(f, pageNo, pageSize);
	}
	...
	public Pagination getPageByChannelPathsForTag(String[] paths, Integer[] siteIds, Integer[] typeIds, Boolean titleImg, Boolean recommend,
		...
		Finder f = byChannelPaths(paths, siteIds, typeIds, titleImg, recommend, title, orderBy);
		f.setCacheable(true);
		return find(f, pageNo, pageSize);
	}
	...
	public Pagination getPageByTopicIdForTag(Integer topicId, Integer[] siteIds, Integer[] channelIds, Integer[] typeIds, Boolean titleImg,
		...
		Finder f = byTopicId(topicId, siteIds, channelIds, typeIds, titleImg, recommend, title, orderBy);
		f.setCacheable(true);
		return find(f, pageNo, pageSize);
	}
	...
	public Pagination getPageByTagIdsForTag(Integer[] tagIds, Integer[] siteIds, Integer[] channelIds, Integer[] typeIds, Integer excludeId,
		...
		Finder f = byTagIds(tagIds, siteIds, channelIds, typeIds, excludeId, titleImg, recommend, title, orderBy);
		f.setCacheable(true);
		return find(f, pageNo, pageSize);
	}
	...
	private Finder byTopicId(Integer topicId, Integer[] siteIds, Integer[] channelIds, Integer[] typeIds, Boolean titleImg, Boolean recommend,
	...
	private Finder byTagIds(Integer[] tagIds, Integer[] siteIds, Integer[] channelIds, Integer[] typeIds, Integer excludeId, Boolean titleImg,
	...
	public int countByChannelId(int channelId) {
		String hql = "select count(*) from Content bean" + " join bean.channel channel,Channel parent"
				+ " where channel.lft between parent.lft and parent.rgt" + " and channel.site.id=parent.site.id" + " and parent.id=:parentId";
	...
	private void appendTypeIds(Finder f, Integer[] typeIds) {
		...
		if (typeIds != null) {
			len = typeIds.length;
			if (len == 1) {
				f.append(" and bean.contentType.id=:typeId");
				f.setParam("typeId", typeIds[0]);
			} else if (len > 1) {
				f.append(" and bean.contentType.id in (:typeIds)");
	...
	private void appendOrder(Finder f, int orderBy) {
		switch (orderBy) {
			...
			case 4:
				...
				break;
			case 5:
				f.append(" order by bean.topLevel desc, bean.contentExt.releaseDate desc");
				break;
			case 6:
				f.append(" order by bean.topLevel desc, bean.contentExt.releaseDate asc");
				break;
			case 7:
				f.append(" order by bean.contentCount.viewsDay desc");
				f.append(", bean.priority desc");
				break;
			case 8:
			...
			case 10:
				...
				break;
			case 11:
				f.append(" order by bean.contentCount.commentsDay desc");
				f.append(", bean.priority desc");
				break;
			case 12:
			...
			case 14:
				...
				break;
			case 15:
				f.append(" order by bean.contentCount.downloadsDay desc");
				f.append(", bean.priority desc");
				break;
			case 16:
			...
			case 18:
				...
				break;
			case 19:
				f.append(" order by bean.contentCount.upsDay desc");
				f.append(", bean.priority desc");
				break;
			case 20:
			...
			case 22:
				...
				break;
			default:
				f.append(" order by bean.topLevel desc, bean.priority desc");
		}
	}

	@Override
	public Integer getMaxPriority() {
		Integer c = (Integer) createQuery("select max(c.priority) from Content c ").uniqueResult();
		if (c == null)
			c = 0;
		return c;
	}
	...
	public Content getPrev(Content bean) {
		String hql = "from Content c where c.priority<? and c.channel=? order by c.priority desc";
		List<Content> list = createQuery(hql, bean.getPriority(), bean.getChannel()).setFirstResult(0).setMaxResults(1).list();
	...
	public Content getNext(Content bean) {
		String hql = "from Content c where c.priority>? and c.channel=? order by c.priority asc";
		List<Content> list = createQuery(hql, bean.getPriority(), bean.getChannel()).setFirstResult(0).setMaxResults(1).list();
		if (list == null || list.size() == 0) {
			return null;
		}
	...
	public long getCountByTimeRange(Integer siteId, TimeRange timeRange) {
		...
		return byTimeRange(f, siteId, "contentExt.releaseDate", timeRange);
	}
}
