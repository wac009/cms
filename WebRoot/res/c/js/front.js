Pn.ns('Core','Core.CheckCode');
Core.showFlash = function(str, wid, hei) {
 ...
 *            上下偏移量。
 */
Core.CheckCode.cssClass = 'j-chkcode';
Core.CheckCode.url = '/CheckCode.svl';
Core.CheckCode = function(input, url, top) {
	this.input = input;
	this.url = url || Core.CheckCode.url;
	this.top = top || 45;
	this.imgLayer = null;
	this.img = null;
	this.event = null;
...
Core.CheckCode.prototype.createHtml = function() {
	this.imgLayer = $('<div/>');
 ...
 * 向上滚动js类
 */
Core.UpRoller = function(rid, speed, isSleep, sleepTime, rollCount, rollSpan,unitHight) {
	this.speed = speed;
	this.rid = rid;
...
Core.UpRoller.prototype.roll = function() {
...
Core.LeftRoller = function(rid, speed, rollSpan) {
	this.rid = rid;
	this.speed = speed;
	this.rollSpan = rollSpan;
	this.proll = $('#roll-' + rid);
...
Core.LeftRoller.prototype.roll = function() {
...
Core.checkUserName = function(username,base) {
	base = base || '';
	$.postJson(base + '/ajax/checkUserName.do', {
		'username' : username
	}, function(data) {
		if (data.success) {
