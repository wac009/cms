...
<html>
  ...
  <body>
  	<div class="navi"><span class="navi_b">当前位置:</span><span class="navi_h">菜单管理 - 修改菜单</span></div>
	<div class="mainData">
	<div class="actInfo"><s:actionerror theme="ccweb"/><s:actionmessage theme="ccweb"/></div>  
	<s:form action="permission_update_edit" cssClass="vldform">
		<s:hidden name="id" />
		...
		<table width="100%" class="table_edit">
			<tr>
				...
				<td class="content">
					<s:if test="parentList==null||parentList.size==0">
						<s:select name="permission.parent.id" disabled="true" list="parentList" cssClass="select"></s:select>
						<span class="help">此菜单式顶级菜单</span>
					</s:if>
					<s:else>
						<s:select name="permission.parent.id" list="parentList" listKey="id" listValue="selectTree" cssClass="select"></s:select>
						<span class="help">指定菜单的上一级</span>
					</s:else>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td class="label">是否菜单：</td>
				<td class="content">
				<s:checkbox name="permission.isMenu" id="permission_isMenu"/><label for="permission_isMenu">是菜单</label>
