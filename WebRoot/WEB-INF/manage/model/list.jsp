	  		...
	  		<div class="data">
				...
				<thead class="pn-lthead">
					<tr>
						...
						<th>名称</th><th>栏目模板前缀</th><th>内容模板前缀</th><th>状态</th><th class="last">操作</th>
					</tr>
				</thead>
				...
				<s:iterator id="row" value="%{list}">
				  <tr name="tr_ids" onmouseover="Pn.LTable.lineOver(this);" onmouseout="Pn.LTable.lineOut(this);" onclick="Pn.LTable.lineSelect(this);">
					...
					<td><s:property value="#row.name"/><s:if test="%{#row.def==1||#row.def==null}"><span class="red">[默认]</span></s:if></td>
					<td><s:property value="#row.tplChannelPrefix"/></td>
					<td><s:property value="#row.tplContentPrefix"/></td>
					...
					<td class="pn-lopt">
						...
						<a href="modelItem_listChannel.jspa?modelId=<s:property value="#row.id"/>&isChannel=true" class="pn-loperator">栏目模型</a>
						<a href="modelItem_listContent.jspa?modelId=<s:property value="#row.id"/>&isChannel=false" class="pn-loperator">栏目模型</a>
					</td>
				</tr>
			    </s:iterator>
			    <s:if test="%{list==null||list.size<=0}">
