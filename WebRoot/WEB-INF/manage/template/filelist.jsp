...
<html>
  ...
  <body>
  	<div id="container">
  	  	<div class="navi">
  	  		<span class="navi_b">当前位置:</span><span class="navi_h">模板管理 - 模板文件列表</span>
  	  		<span class="navi_f">
  	  			<form method="post">
  	  			创建文件夹:<input type="text" name="dirName" size="10" onkeypress="if(event.keyCode==13){$('#dirButton').click();return false;}"/>
				<input id="dirButton" class="btn" type="submit" value="新建" onclick="this.form.action='template_createDir_filelist.jspa?relPath=${relPath}';"/>
  	  			<input type="submit" class="btn" value="创建模板文件" onclick="this.form.action='template_fileadd_filelist.jspa?relPath=${relPath}';"/>
  	  			</form>
  	  		</span>
  	  	</div>
  		<div class="mainData">
	  		...
	  		<div class="data">
				...
				<s:iterator id="row" value="%{subDir}" status="status">
				  <tr name="tr_ids" onmouseover="Pn.LTable.lineOver(this);" onmouseout="Pn.LTable.lineOut(this);" onclick="Pn.LTable.lineSelect(this);">
					...
					<td class="pn-lopt">
						<a href="template_deleteFile_filelist.jspa" onclick="return fileDelete(this,'file-<s:property value="#status.index"/>');" class="pn-loperator">
...
function fileRename(o){
	...
	$.postJson('ajax/template_rename.jspa', {
		'tplName' : $(o).val(),
...
function fileDelete(o,f) {
	if(confirm('您确定删除吗？')) {
		...
		$(o).attr('href',href+'?relPath=${relPath}&tplName='+$('#'+f).val());
		return true;
	} else {
		return false;
	}
