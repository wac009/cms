...
<html>
  ...
  <body>
  	<div class="navi"><span class="navi_b">当前位置:</span><span class="navi_h">投票管理 - 投票主题修改</span></div>
	<div class="mainData">
	<div class="actInfo"><s:actionerror theme="ccweb"/><s:actionmessage theme="ccweb"/></div>
	<s:form action="vote_update_edit" cssClass="vldform">
		<s:hidden name="id" />
		<s:hidden name="voteTopic.id" />
		<s:hidden name="voteTopic.site.id" value="%{webId}"/>
		<s:hidden name="voteTopic.user.id" value="%{adminId}"/>
		<table width="100%" class="table_edit">
			...
			<tr>
				...
				<td class="content">
					...
					<s:iterator value="%{voteTopic.items}" id="row" status="status">
						<div id="dynamicDiv_<s:property value="#status.index"/>">
						标题：<input type="text" class="input" value="<s:property value="#row.title"/>" name="voteItems[<s:property value="#status.index"/>].title" size="50"/>&nbsp;
						票数：<input type="text" class="input" style="width: 30px;" value="<s:property value="#row.voteCount"/>" name="voteItems[<s:property value="#status.index"/>].voteCount" value="0" size="5"/>&nbsp;
						排序：<input type="text" class="input"style="width: 30px;" value="<s:property value="#row.priority"/>"" name="voteItems[<s:property value="#status.index"/>].priority" size="5"/>&nbsp;
...
var lineNum ;
<s:if test="%{voteTopic.items==null}">lineNum=0;</s:if><s:else> lineNum=<s:property value="%{voteTopic.items.size}"/>-1;</s:else>
