...
<html>
  ...
  <body>
	...
	<s:form action="periodicalCatalog_update_edit" cssClass="vldform">
		<s:hidden name="id" />
		<s:hidden name="periodicalId"/>
		<s:hidden name="publicationId"/>
	  	<s:hidden name="periodicalCatalog.id"/>
		...
		<table width="100%" class="table_edit">
			<tr>
				<s:if test="periodical!=null">
					...
					<s:hidden name="periodicalCatalog.publication.id" value="%{publicationId}"/>
				</s:if>
				<s:elseif test="publication!=null">
			...
			<tr>
				...
				<td class="content">
					...
					<s:if test="periodical!=null">
						<s:checkbox name="periodicalCatalog.common" id="periodicalCatalog_iscommon" disabled="true"/><label for="periodicalCatalog_iscommon">通用目录</label>
