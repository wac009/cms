...
<html>
  ...
  <body>
  	<div class="navi"><span class="navi_b">当前位置:</span><span class="navi_h">
		...
		<a href='periodical_list.jspa?publicationId=${publicationId}' style="padding:0;"><s:property value="%{publication.name}"/></a> - 添加</span>
		<span class="navi_f">
	...
	<s:form action="periodical_save_add" cssClass="vldform">
...
function commCtgs(id) {
	$.ajax({
	    type: "post",
	    url : "ajax/periodical_commCtgs.jspa",
	    dataType:"json",
	    data: 'publicationId='+id,
	    success: function(json){
		    var commCtgs=json.jsonRoot.commCtgs;
		    var s="";
		    for(var i=0;i<commCtgs.length;i++){
				s+="<input type='checkbox' name='catalogs' value='"+commCtgs[i].key+"' id='catalogs-"+i+"'/>" +
				"<label for='catalogs-"+i+"' class='checkboxLabel'>"+commCtgs[i].value+"</label>";
			}
		    $("#commCtgs").html(s);
	    },
	    error: function(){
