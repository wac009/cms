...
<html>
  ...
  <body>
  	<div class="navi"><span class="navi_b">当前位置:</span><span class="navi_h">内容管理 - 文章修改</span>
  		<span class="navi_f">
	...
	<s:form action="content_update_edit" cssClass="vldform" id="dataForm">
		<s:hidden name="content.id"/>
		<s:hidden name="content.contentExt.id"/>
		<s:hidden name="content.contentTxt.id"/>
		...
		<table width="100%" class="table_edit">
			<tr>
				...
				<td class="content">
					...
					<div id="channelsContainer">
						<s:iterator id="chnl" value="%{content.channelsWithoutMain}">
							<div style="padding:0">
								<s:iterator id="node" value="#chnl.nodeList" status="stat">
									<span><s:property value="#node.name"/> </span>
									<s:if test="!#stat.last"> > </s:if>
								</s:iterator>
								<a href="javascript:void(0);" onclick="$(this).parent().remove();" class="pn-opt"> 删除 </a>
								<input type="hidden" name="channelIds" value="<s:property value="#chnl.id"/>"/>
							</div>
						</s:iterator>
					</div>
				</td>
			...
			<tr>
				...
				<td class="content">
					...
					<span class="help">若作为超链接以下设置的内容将失效</span>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="content">
					<s:textfield name="content.contentExt.editorName" cssClass="input" />
			...
			<tr>
				...
				<td class="content">
					<s:textfield name="releaseTime" cssClass="input Wdate" cssStyle="padding:2px 2px 1px 2px;" onclick="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
			...
			<!-- 
			<tr>
				<td class="label">Tag标签：</td>
				<td class="content">
					<s:textfield name="tagStr" cssClass="input" />
					<span class="help" style="margin-right:20px;">多个用","分隔</span>
					置顶设置：<s:select cssStyle="width:100px;border:1px solid #CCC;" name="content.topLevel" list="#{'0':'不置顶','6':'置顶1','5':'置顶2','4':'置顶3','3':'置顶4','2':'置顶5','1':'置顶6'}" />
				</td>
			</tr> -->
			<tr>
			...
			<!--  
			<tr id="typeImgDiv">
				<td class="label">类型图：</td>
				<td class="content">
					<div class="float_left">
						图片路径<s:textfield name="content.contentExt.typeImg" id="uploadImgPath3" cssClass="input" /><input type="button" value="清空" onclick="clearImg(3);" class="btn"/><br/>
						本地上传<span id="ifc3"><input type="file" id="imgFile3" size="20" class="input"/></span>水印<s:checkbox name="mark"  value="true"/>
						<input type="button" value="上传" onclick="uploadImg(3);" class="btn"/><br/>
						图片大小：宽<input type="text" id="zoomWidth3" value="139" size="5" class="input" style="width:40px"/> 
						高<input type="text" id="zoomHeight3" value="139" size="5" class="input" style="width:40px"/>
						<input type="checkbox" id="zoom3" value="true" checked="checked"/>自动压缩图片
						<input type="button" value="裁剪" onclick="imgCut(3);" class="btn"/>
					</div>
					<div class="float_left" style="margin-left:15px;">
						<img id="preImg3" alt="预览区" noResize="true" onerror="javascript:this.src='<%=r_m%>/img/defaultpic.gif'"  srcRoot="${web.uploadPath}" src="${web.uploadPath}<s:property value="content.titleImg"/>" style="width:100px;height:70px;background-color:#CCCCCC;border:1px solid #333"/>
					</div>
				</td>
			</tr>
			-->
			...
			<tr id="picSet">
				...
				<td class="content">
					...
					<s:if test="%{content.pictures!=null&&content.pictures.size>0}">
					<s:iterator id="p" value="%{content.pictures}" status="stat">
						<table id="picTable<s:property value="#stat.index"/>" border="0" style="float:left;margin:5px 5px 0 0;padding:2px;">
						<tr>
							<td>
								<div><input type="text" id="uploadImgPath<s:property value="#stat.index"/>" name="picPaths" value="<s:property value="#p.imgPath"/>" class="input" style="width:135px;margin-right:2px;"/> 
								<input type="button" onclick="$('#picTable<s:property value="#stat.index"/>').remove();" value="删除" class="btn" /></div>
								<div><span id="ifc<s:property value="#stat.index"/>">上传<input type="file" id="imgFile<s:property value="#stat.index"/>" size="10" class="input" style="width:140px"/></span> <input type="button" value="上传" onclick="upload(<s:property value="#stat.index"/>);" class="btn"/></div>
								<div><textarea style="width:180px;height:60px;" class="input txtArea" name="picDescs"><s:property value="#p.description" /></textarea></div>
							</td>
							<td><img id="preImg<s:property value="#stat.index"/>" onerror="javascript:this.src='<%=r_m%>/img/defaultpic.gif'"  srcRoot="${web.uploadPath}" src="${web.uploadPath}<s:property value="#p.imgPath"/>" alt="预览" noResize="true" style="width:110px;height:110px;background-color:#ccc;border:1px solid #333"/></td>
						</tr>
						</table>
					</s:iterator>
					</s:if><s:else>
						<table id="picTable2" border="0" style="float:left;margin:5px 5px 0 0;padding:2px;">
							<tr>
								...
								<td><img id="preImg2" srcRoot="${web.uploadPath}" alt="预览" onerror="javascript:this.src='<%=r_m%>/img/defaultpic.gif'"  noResize="true" style="width:110px;height:110px;background-color:#ccc;border:1px solid #333"/></td>
							</tr>
						</table>
					</s:else>
					...
					<textarea id="picTable" style="display:none;">
						<table id="picTable{0}" border="0" style="float:left;margin:5px 5px 0 0;padding:2px;">
							<tr>
								...
								<td><img id="preImg{0}" srcRoot="${web.uploadPath}" alt="预览" noResize="true" style="width:110px;height:110px;background-color:#ccc;border:1px solid #333"/></td>
							</tr>
						</table>
					</textarea>
					<script type="text/javascript">
					var picIndex = <s:property value="%{content.pictures==null?3:content.pictures.size+4}"/>;
			...
			<tr id="attachment">
				...
				<td class="content" >
					...
					<s:if test="%{content.attachments!=null&&content.attachments.size>0}">
						<s:iterator id="attach" value="%{content.attachments}" status="stat">
							<table>
								<tr id="attachTr<s:property value="#stat.index"/>" class="noborder">
									...
									<td style="border:none;border-top:1px solid #ccc;">
										<input type="hidden" id="attachmentFilenames<s:property value="#stat.index"/>" name="attachmentFilenames" value="<s:property value="#attach.filename"/>"/>
									</td>
								</tr>
							</table>
							<script type="text/javascript">
			...
			<tr>
				<td class="content" colspan="2">
					<fck:editor instanceName="content.contentTxt.txt" height="600px;">
						<jsp:attribute name="value">
							<s:property value="%{content.contentTxt.txt}" escape="false" /> 
						</jsp:attribute>
					</fck:editor>
				</td>
			</tr>
