  ...
  <body>
  	<div id="container">
  	  	<div class="navi"><span class="navi_b">当前位置:</span>
  	  		<span class="navi_h">广告管理 - 广告列表</span>
  	  		<span class="navi_f">
	  	  		<a href='advertising_list.jspa' style="padding:0 5px;">刷新</a>
	  	  		<a href='advertising_add.jspa' style="padding:0 5px;">添加</a>
  		...
  		<s:form id="dataForm">
	  		...
	  		<div class="data">
				...
				<tr>
					...
					<th>ID</th><th>名称</th><th>版位</th><th>类型</th><th>权重</th><th>点击/展现=点击率</th><th>启用</th><th class="last">操作</th>
				</tr>
				</thead>
				...
				<s:iterator id="row" value="%{pagination.list}">
				  <tr name="tr_ids" onmouseover="Pn.LTable.lineOver(this);" onmouseout="Pn.LTable.lineOut(this);" onclick="Pn.LTable.lineSelect(this);">
					...
					<td><s:property value="#row.adspace.name"/></td>
					<td><s:property value="#row.category"/></td>
					<td><s:property value="#row.weight"/></td>
					<td><s:property value="#row.clickCount"/>/<s:property value="#row.displayCount"/>=<s:property value="#row.percent"/>%</td>
					<td><s:if test="%{#row.enabled==0||#row.enabled==null}"><img src="<%=r_m%>/img/remove.gif" alt="禁用" />禁用</s:if>
					<s:else><img src="<%=r_m%>/img/check.gif" alt="启用" />启用</td></s:else>
					<td class="pn-lopt">
